...
could not ship the locally repaired models. This follow-up must contain all five
...
| Panel Van | Gap between sliding cargo panel and rear fixed quarter, mismatched roof/header | Continuous cargo upper side, extended roof, rear posts/header, solid barn doors and moving center seal |
...
packet or entity-data fields. Matched server/client versions remain recommended.
...
LIBGL_ALWAYS_SOFTWARE=1 ALSOFT_DRIVERS=null xvfb-run -a -s '-screen 0 1600x1000x24' \
  ./gradlew --no-daemon -PwithGameTests runClientShellSmoke
python tools/ci/checks.py shell --log build/ci/client.log --report build/ci/shell.json
```

Capture the preceding client's output into `build/ci/client.log` when running the last
checker locally. Normal PR/main CI does this automatically and blocks the required
checks if any body/shell job fails. A PR run does not publish a release; the existing
release job runs only after a push to main and all required jobs pass.
...
seams in both directions. Negative controls deliberately remove/reverse/duplicate
faces and leave gaps to demonstrate that the checker fails those cases.
...
The recovered source was regenerated locally for this delivery: 8 topology negative
controls, 2,682 geometry/seam assertions across 748 checked shell chunks, 245
