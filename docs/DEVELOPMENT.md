...
The committed APA2 mesh contains **788 material/part batches and 287,040 triangles across all alternatives**, compressed to **3,487,989 bytes**. Most alternatives are mutually exclusive, and the in-world engine bay is culled when the hood is closed. Each selected hardware choice has distinct vertex positions, verified for every family. Runtime metadata selects family, service slot/variant and compressor. Body paint is tinted at draw time. Mesh data is cached on resource reload. See [runtime-assets.json](runtime-assets.json) and the [0.6.1 body/rig/collision changes](BODY_AND_COLLISION.md).
