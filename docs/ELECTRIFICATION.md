# Electric and series-hybrid vehicles (0.6.0 alpha)

Minecraft 1.21.1 / NeoForge 21.1.249 / Java 21. Client and server must use the same mod version (network protocol 8). Existing combustion sedans retain their original drivetrain; old saves do not silently convert into EVs. Back up a world before testing an alpha.
...
Hybrids use a **parallel mechanical engine path** through the clutch and five-speed gearbox, with electric assistance and fuel-powered generation. Auto/Sustain taper assistance at highway speed and retain a propulsion reserve; steady cruise can divert actual crank torque to recharge. Electric Only keeps the engine off. See [hybrid control, reserve bands and measured performance](HYBRID_AND_PERFORMANCE.md) for the current supervisor and its limitations.

For pack service: park, switch READY off, unplug, open the hood fully, carry one matching traction pack and select **Replace pack** on Electric. The incoming pack is consumed and the old pack is returned, with both packs' condition and energy preserved. The service action is server-authoritative and subject to vehicle ownership and distance checks. The complete pack is the service unit in this alpha; individual cells, inverter swaps and contactor replacement are not implemented.

## ElectricalAge charging

The optional integration uses the public `mods.eln.api.v1.electrical.ElectricalIntegration` API, pinned and tested against companion commit `99f911d7a6c5a501345fde542cd879d8706b78af`. Install ElectricalAge's 1.21.1 NeoForge JAR and **Kotlin for Forge 5.12.0** on both sides. No hard dependency is required to load AutoPropulsion by itself.
...
Pair deliberately: use a **Vehicle Charging Cable** on the charger, then on an owned, stationary EV/PHEV within six blocks. The link is dimension-checked and one charger serves one vehicle. READY is blocked while connected. A successful connection installs one cable. Right-click the charger to open its complete live screen; use **Unplug / recover cable**, or sneak-right-click either endpoint with an empty hand. A full inventory drops the returned cable. After unload/reload the connection expires but the item stays recoverable in the charger. Breaking the charger drops it. No forced chunk loading or proximity auto-charging is used. [Screenshots, controls and recovery details](CHARGER_AND_CABLE.md).
...
The server advances traction at four 12.5 ms substeps per tick. Electric torque feeds the same independent tire forces, mass-adjusted body and suspension, steering, differential and service/handbrake paths as combustion. Direction is explicitly selected, never inferred from a backwards slide. Shared driveline parts interrupt motor drive when missing or failed. The pack and each installed motor, inverter, reduction gear, CV shaft, differential, HV harness, contactor, 12 V converter and hybrid generator are serviceable. Their stored condition controls the corresponding torque or electrical path. AWD has separate front and rear drive units sharing the vehicle rating; it does not double the rated output. Drive-unit losses heat the coolant circuit; coolant quantity and pump/radiator/fan condition affect heat rejection. A simplified 10 A accessory recharge is included in the READY power budget; it uses the existing 12 V battery state. Energy uses joules, power watts, time seconds and capacity kWh (`1 kWh = 3,600,000 J`), with no accelerated recharge multiplier. The battery is a lumped Thevenin model with SOC-dependent OCV, resistive sag/heat, current limits, temperature derating, charge taper and throughput-based aging. Motor/generator maps are calibrated approximations. Midpoint drive-shaft work accounts for energy required to launch from rest; regenerative energy is bounded by actual braking work and pack acceptance. Conversion losses and fuel-backed generation are accounted separately.

Not implemented: per-cell electrochemistry, cell imbalance, thermal runaway/fire propagation, isolation testing, a detailed DC-DC converter or 12 V circuit solver, physical contactor timing, parallel/power-split hybrids, vehicle-to-grid, an AC phase solver, licensed/OEM motor maps or EV-specific motor sound recordings. These limitations must not be presented as fully simulated systems.
...
The required CI workflow runs combustion and electric simulation tests, all 12 electric/layout server cases, the original 294 combustion/layout cases, 192 workshop page/scale cases, native electric driving/charging, and ElectricalAge native integration. Release is blocked unless every job passes on the merged source. Exact run evidence is attached to each tested release.
