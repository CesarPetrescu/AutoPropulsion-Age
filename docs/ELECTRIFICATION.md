...
Minecraft 1.21.1 / NeoForge 21.1.249 / Java 21. Client and server must use the same mod version (network protocol 7). Existing combustion sedans retain their original drivetrain; old saves do not silently convert into EVs. Back up a world before testing an alpha.
...
The required CI workflow runs combustion and electric simulation tests, all 12 electric/layout server cases, the original 294 combustion/layout cases, 156 workshop page/scale cases, native electric driving/charging, and ElectricalAge native integration. Release is blocked unless every job passes on the merged source. Exact run evidence is attached to each tested release.
