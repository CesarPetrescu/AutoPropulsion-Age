# Playing AutoPropulsion Age 0.2.0-alpha

This alpha lets you place, drive and customize one sedan. It includes five garage tabs, a driving HUD, survival recipes, fuel, repairs and saved configurations. The larger simulation and workshop design in [the original requirements](modular-car-requirements.md) remains a roadmap.

## Install

1. Install **Minecraft Java Edition 1.21.1** with **NeoForge 21.1.249** and **Java 21**. This is the tested combination.
2. Download [autopropulsion-age-0.2.0-alpha.jar](https://github.com/CesarPetrescu/AutoPropulsion-Age/raw/refs/heads/main/downloads/autopropulsion-age-0.2.0-alpha.jar).
...
4. Replace the previous AutoPropulsion JAR when upgrading; keep only one version installed. Existing 0.1 cars migrate to stock I4 engine components.
...
| Engine | Seven engine families, stock/sport grades, six service slots, natural/turbo/supercharger layouts, internals inspection and temperature |
...
Configuration, engine family and components, owner, paint, fuel, condition, temperature, tune, hood and lamp state save with the entity. Reloaded cars start parked with the engine off. The all-panels control opens all six together, and the hood also has an independent control. There is one usable seat.
...
- The runtime uses selected stock and sport assets. The seven engine families and complete induction kits are implemented; the full 471-entry Blender kit also includes workshop and driveline assets that remain authoring assets. Texture atlases, liveries, LODs and large-fleet performance work remain.
