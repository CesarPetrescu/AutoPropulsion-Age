...
/** Opt-in visual/in-world smoke driver. It uses the same validated packets as ordinary input. */
public final class ClientSmoke {
    private static int ticks, idleTicks;
    private static Vec3 start;
    private static boolean menuSeen;
    private static String profile;
    private ClientSmoke() {}
    private static void shot(Minecraft mc,String name) {
        Screenshot.grab(mc.gameDirectory,"apa-"+profile+"-"+name+".png",mc.getMainRenderTarget(),message -> AutoPropulsion.LOG.info("APA_SCREENSHOT {}",message.getString()));
    }
    public static void tick(Minecraft mc) {
        if (mc.player==null || !(mc.player.getVehicle() instanceof VehicleEntity car)) {
            if (++idleTicks>2400) { AutoPropulsion.LOG.error("APA_CLIENT_SMOKE_FAILED: no mounted vehicle"); mc.stop(); }
            return;
        }
        if (ticks++==0) {
            profile=System.getProperty("apa.profile","fancy"); start=car.position();
            ...
            AutoPropulsion.LOG.info("APA_CLIENT_SMOKE_CONNECTED profile={} vehicle={}",profile,car.getId());
        }
        ...
        if (ticks==290) {
            ...
            if (distance<5 || !Float.isFinite(car.rpm())) { AutoPropulsion.LOG.error("APA_CLIENT_SMOKE_FAILED: driving did not advance the server-owned vehicle"); mc.stop(); return; }
        }
        if (ticks==340) { mc.player.setYRot(150); mc.player.setXRot(22); shot(mc,"hood"); }
        ...
        if (ticks==410) {
            ...
            if (!menuSeen) { AutoPropulsion.LOG.error("APA_CLIENT_SMOKE_FAILED: workshop menu did not open"); mc.stop(); return; }
            shot(mc,"diagnostics");
        }
        ...
        if (ticks==520) {
            AutoPropulsion.LOG.info("APA_CLIENT_SMOKE_COMPLETE profile={} menu={}",profile,menuSeen);
            mc.stop();
        }
    }
}
