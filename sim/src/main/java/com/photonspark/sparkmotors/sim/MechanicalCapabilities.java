...
public final class MechanicalCapabilities {
    public static boolean canCrank(MechanicalState m){return CircuitPhysics.batteryCharge(m)>1&&m.capability("electrical.starter")>.15&&m.capability("electrical.wiring")>.2&&m.capability("electrical.fuse")>.2&&m.capability("engine.internals")>.02;}
    ...
    public static double combustion(MechanicalState m,double rpm){
        ...
        return m.capability("engine.intake")*m.capability("engine.fuel")*m.capability("engine.ignition")*(1-internals.wear()*.5)*regularity;
    }
}
