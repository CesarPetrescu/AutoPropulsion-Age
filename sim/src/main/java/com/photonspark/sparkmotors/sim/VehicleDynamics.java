...
public final class VehicleDynamics {
    ...
    public record Input(double throttle, double steer, boolean brake, boolean reverse, boolean handbrake) {
    ...
    public record Setup(int config, int limiter, double finalDrive, EngineFamily family, int engineParts, double temperature, VehicleCondition.Effects condition) {
        public Setup(int config,int limiter,double finalDrive,EngineFamily family,int engineParts,double temperature){this(config,limiter,finalDrive,family,engineParts,temperature,new VehicleCondition().effects(config,engineParts));}
        public Setup(int config,int limiter,double finalDrive){this(config,limiter,finalDrive,EngineFamily.I4,EnginePart.stock(),90);}
        public Setup { config = Assembly.sanitize(config); limiter = Math.clamp(limiter, 4000, 7000); finalDrive = clamp(finalDrive, 2.8, 4.8); family=family==null?EngineFamily.I4:family;engineParts=EnginePart.sanitize(engineParts);temperature=clamp(temperature,20,150); condition=condition==null?new VehicleCondition().effects(config,engineParts):condition; }
    }
    public record State(double speed, double rpm, int gear, double fuel, double yawDelta, double fuelUsed, double slip) {
        public State(double speed,double rpm,int gear,double fuel,double yawDelta,double fuelUsed){this(speed,rpm,gear,fuel,yawDelta,fuelUsed,0);}
    }
    ...
    public static State step(double speed, double fuel, boolean ignition, Input in, Setup setup, double grip, boolean grounded, double dt) {
        dt=clamp(dt,0,.25); grip=clamp(grip,0,2)*setup.condition.grip();
        ...
        boolean running = ignition && fuel > 0 && Assembly.ENGINE.variant(setup.config) > 0 && EnginePart.ready(setup.engineParts) && setup.condition.canRun();
        boolean drive = running && Assembly.canDrive(setup.config);
        double force = 0, slip = 0;
        if (running) {
        ...
        if (drive && grounded && (!in.reverse || Math.abs(speed) < 13)) {
            force = EngineBuild.torque(rpm,setup.family,Assembly.ENGINE.variant(setup.config),setup.engineParts,setup.limiter,setup.temperature,setup.condition.boost()) * setup.condition.power() * ratio * .86 / WHEEL_RADIUS * in.throttle * (in.reverse ? -1 : 1);
            double tireGrip = grip * (Assembly.WHEELS.variant(setup.config) == 2 ? 1.17 : 1);
            double limit=MASS*9.81*tireGrip*.56;
            slip=clamp((Math.abs(force)-limit)/Math.max(1,limit),0,1);
        ...
        if (grounded) {
            ...
            double steering = clamp(in.steer*setup.condition.steering()+setup.condition.pull(),-1,1) * .49 * response / (1 + Math.abs(next) * .045);
            yaw = next / WHEELBASE * Math.tan(steering) * dt * Math.min(1, grip);
        }
        return new State(clamp(next, -14, 65), rpm, gear, Math.max(0, fuel - used), yaw, used, grounded?slip:0);
    }
