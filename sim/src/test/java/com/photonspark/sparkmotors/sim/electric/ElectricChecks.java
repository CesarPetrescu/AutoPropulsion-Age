...
public final class ElectricChecks {
    ...
    public static void driving(){
        for(var type:Powertrain.values())if(type.electric()){
            var state=ElectricDynamics.State.initial(type,.65);var engine=EnginePhysics.State.stopped(90,100);double fuel=40,speed=0,start=state.battery().energyJ();var wheels=WheelDynamics.State.stopped();var trans=TransmissionPhysics.State.stopped();
            for(int i=0;i<2400;i++){
                var r=ElectricDynamics.step(type,state,ElectricDynamics.Mode.ELECTRIC_ONLY,speed,fuel,engine,wheels,trans,true,new VehicleDynamics.Input(1,0,false,false,false),setup(),new double[]{1,1,1,1},new boolean[]{true,true,true,true},new double[4],false,.0125,20);
                speed=r.road().speed();fuel=r.road().fuel();engine=r.road().engine();state=r.electric();wheels=r.road().wheels();trans=r.road().transmission();
            }
            ...
            for(int i=0;i<1600;i++){
                var r=ElectricDynamics.step(type,state,ElectricDynamics.Mode.ELECTRIC_ONLY,speed,fuel,engine,wheels,trans,true,new VehicleDynamics.Input(0,0,true,false,false),setup(),new double[]{1,1,1,1},new boolean[]{true,true,true,true},new double[4],false,.0125,20);
                speed=r.road().speed();engine=r.road().engine();state=r.electric();wheels=r.road().wheels();trans=r.road().transmission();
            }
