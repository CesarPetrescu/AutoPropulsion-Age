...
public final class ChargerBlock extends Block implements EntityBlock {
    ...
    public ChargerBlock(Properties properties,ChargingModel.Tier tier){super(properties.noOcclusion());this.tier=tier;registerDefaultState(stateDefinition.any().setValue(FACING,Direction.NORTH));}
    @Override protected void createBlockStateDefinition(StateDefinition.Builder<Block,BlockState> builder){builder.add(FACING);}
    @Override public BlockState getStateForPlacement(BlockPlaceContext context){return defaultBlockState().setValue(FACING,context.getHorizontalDirection().getOpposite());}
    @Override protected VoxelShape getShape(BlockState state,BlockGetter level,BlockPos pos,CollisionContext context){
        return ChargerShapes.get(tier,state.getValue(FACING));
    }
    // The casing does not fill its block cell. In particular it must not hide the
    // supporting block around its inset foot or a neighboring solid block.
