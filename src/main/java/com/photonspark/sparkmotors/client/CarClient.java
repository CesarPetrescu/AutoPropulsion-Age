...
public final class CarClient {
    ...
    private static void hud(GuiGraphics g){
        ...
        float meter=car.powertrain().electric()?car.stateOfCharge():Math.min(1,car.rpm()/car.limiter());
        g.fill(x+102,y+38,x+w-10,y+43,0xFF2A3C49);g.fill(x+102,y+38,x+102+(int)(128*meter),y+43,car.powertrain().electric()?(meter<.1?0xFFF17C56:0xFF31C6C9):(car.rpm()>car.limiter()*.9?0xFFF17C56:0xFF31C6C9));
        g.drawString(mc.font,car.powertrain().electric()?String.format(java.util.Locale.ROOT,"SOC %.1f%%",car.stateOfCharge()*100):String.format(java.util.Locale.ROOT,"FUEL %.1f L",car.fuel()),x+10,y+53,(car.powertrain().electric()?car.stateOfCharge()<.1:car.fuel()<5)?0xFFFFA45C:0xFFD1E2E8,false);
        ...
        g.drawCenteredString(mc.font,car.powertrain().electric()?"W drive  S brake/regen  Z reverse  G garage":"W drive  S brake  C clutch  Z reverse  G garage",g.guiWidth()/2,y+89,0xFFDAE6EC);
    }
}
