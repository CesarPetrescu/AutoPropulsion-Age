...
public record EngineCraftingRecipe(ShapedRecipe delegate) implements CraftingRecipe {
    ...
    @Override public ItemStack assemble(CraftingInput input,HolderLookup.Provider registries){
        ...
        var combined=crate?PowertrainTopology.fresh(Powertrain.COMBUSTION,DriveConfig.preset(DriveConfig.Layout.RWD),EngineFamily.I4,Assembly.stock(),EnginePart.stock()):null;
        for(int i=0;i<input.size();i++){
            ...
            if(donor.getItem() instanceof EngineItem){
                ...
                if(crate){var bundle=MechanicalData.bundle(donor,s->s.assembly()==com.photonspark.sparkmotors.sim.Assembly.ENGINE,com.photonspark.sparkmotors.sim.Assembly.stock(),EngineItem.parts(donor));bundle=InternalMechanics.migrateBundle(bundle,EngineFamily.I4);combined=combined.replace(s->s.assembly()==com.photonspark.sparkmotors.sim.Assembly.ENGINE,bundle,true);}
                ...
                if(crate){
                    for(var group:java.util.List.of(com.photonspark.sparkmotors.sim.Assembly.WHEELS,com.photonspark.sparkmotors.sim.Assembly.TRANSMISSION))
                        if(donor.is(AutoPropulsionAge.partItem(group,1)))combined=combined.replace(s->s.assembly()==group,PowertrainTopology.migrate(mechanical,Powertrain.COMBUSTION,DriveConfig.preset(DriveConfig.Layout.RWD),EngineFamily.I4,Assembly.stock(),EnginePart.stock()),false);
                }else{
                    // Reworking a used single part changes its specification, never its identity or condition.
                    if(mechanical.parts().size()==1||mechanical.get("engine.internals")!=null){String key=mechanical.get("engine.internals")!=null?"engine.internals":mechanical.parts().keySet().iterator().next();var p=mechanical.get(key);String item=net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(result.getItem()).getPath();
                        mechanical=mechanical.with(key,new com.photonspark.sparkmotors.sim.PartInstance(p.id(),item,p.wear(),p.damage(),p.faults(),p.reserve(),p.temperature()));}
