...
with gzip.open(repo/'src/main/resources/assets/sparkmotors/models/entity/sedan.mesh.gz','rb') as f:
    ...
    for _ in range(count):
        ...
        chunks.append(dict(name=name,group=group,hinge=hinge,pivot=(px,py,pz),angle=angle,family=family,slot=slot,induction=induction,vertices=vertices))
...
fixed={name:bvh([c for c in chunks if c['name']==name]) for name in ['battery','radiator','coolant_reservoir']}
checks=[]
for family in range(7):
    for induction in range(3):
        ...
        cores=[c for c in engine if not c['name'].startswith(('service_','turbo_header_','boost_plumbing_')) and c['name'] not in ['turbocharger','supercharger','battery','radiator','radiator_fan','coolant_reservoir']]
        compressors=[c for c in engine if c['name'] in ['turbocharger','supercharger']]
...
report={'scope':'Exact runtime triangle intersections: 21 layouts × 5 hood positions, compressors versus core, boost plumbing versus battery/radiator/reservoir. Intended internal mating surfaces are excluded.',
        'layouts':21,'hood_positions':[0,.25,.5,.75,1],'passed':sum(c['passed'] for c in checks),'failed':sum(not c['passed'] for c in checks),'checks':checks}
